...
#         x_tst_sp = np.expand_dims(x_tst[:,lala,lonind[hh],0],axis=1)
        ...
        #     #save location:#     ##     ##     ##     #
        newdir = '/glade/work/wchapman/AnEn/CNN/Coastal_Points_LogNormal/perm_models/' +fcast+'/EMOS_'+tstyr
        if not os.path.exists(newdir):
            os.makedirs(newdir)
    
        Wsave_name = newdir+'/EMOS_CRPS_val_'+ valyr+'_test_'+tstyr+'_lat'+str(latsDO[hh])+'_lat'+str(lonsDO[hh]+360)+'.ckpt'
        #     ##     ##     ##     ##     ##     ##     #
