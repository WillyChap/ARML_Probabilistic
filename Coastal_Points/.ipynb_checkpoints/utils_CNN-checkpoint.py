...
NETCDF_PREDICTOR_NAMES = [
    NETCDF_IVT_NAME, NETCDF_Psfc_NAME, NETCDF_U500_NAME, NETCDF_V500_NAME, NETCDF_Z500_NAME, NETCDF_IWV_NAME
]

PREDICTOR_NAMES = [
    IVTgfs, psfc, u500, v500, z500, IWV
]
...
# Machine-learning constants... you can alter these.
L1_WEIGHT = 0.
L2_WEIGHT = 0.001
...
def read_image_file(netcdf_file_name,targ_LATinds=None,targ_LONinds=None):
    ...
    if targ_LATinds is None:
        ...
        post_matrix = predictor_matrix[:,targ_LATinds,targ_LONinds,indPOST]
        post_matrix = numpy.reshape(post_matrix,(post_matrix.shape[0],-1))
...
def _update_normalization_params(intermediate_normalization_dict, new_values):
    ...
    new_values[new_values==0]=numpy.nan
    
    these_means = numpy.array([
        intermediate_normalization_dict[MEAN_VALUE_KEY], numpy.nanmean(new_values)
    ])
...
def deep_learning_generator(netcdf_file_names, num_examples_per_batch,
    ...
    while True:
        ...
        full_target_matrix = numpy.expand_dims(full_target_matrix,axis=-1)
...
def deep_learning_generator_nordnm(netcdf_file_names, num_examples_per_batch,
                            normalization_dict,normalization_dict_targ,targ_LATinds=None,
                           targ_LONinds=None):
    """Generates training examples for deep-learning model on the fly.
    E = number of examples 
...
#             batch_indices, size=num_examples_per_batch, replace=False)
        ...
        #time 
        ff = netCDF4.Dataset(netcdf_file_names[file_index])
        nctime = ff.variables['time'][:] # get values
        t_unit = ff.variables['time'].units # get unit  "days since 1900-01-01T00:00:00Z"
        t_cal = ff.variables['time'].calendar
        time_values = netCDF4.num2date(nctime,units = t_unit,calendar = t_cal)

        yield (predictor_matrix, target_values,time_values)
...
def train_cnn_ms(
        ...
        num_training_batches_per_epoch, output_model_file_name,
        validation_file_names=None, num_validation_batches_per_epoch=None,
    targ_LATinds=None, targ_LONinds=None,pretrained=None):
    ...
    # with tensorflow.device("/device:GPU:0"):
    #     K.get_session().run(tensorflow.global_variables_initializer()) #don't run if pretrained.

    #     if pretrained is not None:     # load a pretrained model:            
    #         cnn_model_object.load_weights(pretrained)
...
def get_latlon_ind(latlonfolder):
    ...
    latind=numpy.array([])
    lonind=numpy.array([])
    for ff in glob.glob('/data/downloaded/Forecasts/Machine_Learning/glucidi_WRF/LATLON/lat*lon*'):
        ff2 = ff.split('lat')[1]
        ff3 = (ff2.split('lon'))
        latfind = numpy.append(latfind,float(ff3[0]))
        lonfind = numpy.append(lonfind,float(ff3[1])-360)

    latm = netCDF4.Dataset('/data/downloaded/Forecasts/Machine_Learning/glucidi_WRF/F024/F024_MERRA_wrfcf_d01_2006-01-15_00_00_00.nc').variables['lat'][:]
    lonm = netCDF4.Dataset('/data/downloaded/Forecasts/Machine_Learning/glucidi_WRF/F024/F024_MERRA_wrfcf_d01_2017-03-19_00_00_00.nc').variables['lon'][:]
...
def load_forecast(fcast,yearstart):
    
    if yearstart not in pandas.date_range("1985", periods=32, freq="Y").year.astype('str').tolist():
        raise NameError('forecast must be between 1985 and 2016')
    
    df = pandas.read_pickle('/glade/scratch/wchapman/Reforecast/models/NN_CRPS/'+fcast+'/Reforecast_out/NN_CRPS_CNNPP'+'_StartingYear'+yearstart+'_FINETUNEref2.pkl')
    Pre_m = numpy.array(df['Model'])
    Pre_s = numpy.zeros(len(Pre_m)) 
    Post_m = numpy.array(df['IVTmean'])
    Post_s = numpy.array(df['IVTstd'])
    Obs = numpy.array(df['OBS'])

    return df, Pre_m, Pre_s, Post_m, Post_s, Obs



def load_NN_forecast(fcast,yearstart):
    
    if yearstart not in pandas.date_range("1985", periods=32, freq="Y").year.astype('str').tolist():
        raise NameError('forecast must be between 1985 and 2016')
    
    df = pandas.read_pickle('/glade/scratch/wchapman/Reforecast/models/NN_CRPS/'+fcast+'/Reforecast_out/NN_CRPS_NNPP'+'_StartingYear'+yearstart+'_FINETUNEref2.pkl')
    Pre_m = numpy.array(df['Model'])
    Pre_s = numpy.zeros(len(Pre_m)) 
    Post_m = numpy.array(df['IVTmean'])
    Post_s = numpy.array(df['IVTstd'])
